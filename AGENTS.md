...
- **Android APK**: The APK is a thin WebView shell around the WASM build. Because Chromium blocks `fetch()` on `file://` origins, `MainActivity` intercepts requests to `https://appassets.androidplatform.net/assets/*` and serves `dist/web` from the APK assets. The `packaging/android/mario.keystore` is committed and self-signed (zero value, for sideload convenience only); if distributing via the Play Store, replace it and use real GitHub Actions secrets.- **Check `git status` before `git add -A` on the main checkout** — the user keeps WIP test files there; sweeping them up has shipped half-done tests before.
